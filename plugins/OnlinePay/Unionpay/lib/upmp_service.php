...
class UpmpService {
    ...
    static function trade($req, &$resp) {
		...
		if(SYS_DEBUG_MODE)
			$whb_trade_url="http://202.101.25.178:8080/gateway/merchant/trade";
		else
			$whb_trade_url="https://mgate.unionpay.com/gateway/merchant/trade";
		//$respString = post(upmp_config::$upmp_trade_url, $nvp);			
		$respString = post($whb_trade_url, $nvp);
		sys_log("订单推送请求报文：".$nvp);
		sys_log("订单推送应答报文：".$respString);
    ...
    static function query($req, &$resp) {
    	$nvp = self::buildReq($req);
		if(SYS_DEBUG_MODE)
			$whb_query_url="http://202.101.25.178:8080/gateway/merchant/query";
		else
			$whb_query_url="https://mgate.unionpay.com/gateway/merchant/query";
		//$respString = post(upmp_config::$upmp_query_url, $nvp);	
		$respString = post($whb_query_url, $nvp);		
		sys_log("交易查询请求报文：".$nvp);
		sys_log("交易查询应答报文：".$respString);
    	return self::verifyResponse($respString, $resp);
    }
