...
class LoginAction extends AdminAction{
    ...
    public function index(){
        if(IS_POST){
            ...
            //查询数据
            $admin_r = $this -> get_list_bysql("select * from sys_shop where username='$username' ");
            //验证账号
            if(!$admin_r) sys_out_fail("账号不存在");
            if($password != $admin_r[0]['password']) sys_out_fail("密码不正确");
            if($admin_r[0]['validflag'] == 2)  sys_out_fail("账号已被冻结");
            //更新登录信息
            $this -> do_execute("update sys_shop set login=login+1,lastlogintime='$regdate',lastloginip='$ip' where id=".$admin_r[0]['id']);
            //保存session值
            $auth = array(
                'uid' => $admin_r[0]['id'],
                'username' => $admin_r[0]['username'],
                'realname' => $admin_r[0]['name'],
