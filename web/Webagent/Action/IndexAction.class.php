...
class IndexAction extends BaseAction
{
    ...
    public function index(){
        if(UID){
            ...
            if($role_id == 1){
                $menu_list = $this -> _cascade_get(0,'sys_menu');
            }
            else{
                $menu_ids = $this -> get_one_bysql("select GROUP_CONCAT(menu_id) from sys_role_priv where role_id=$role_id ");
                $menu_where = $menu_ids ? "id in ($menu_ids )" : "false";
                $menu_list = $this -> _cascade_get(0,'sys_menu',$menu_where);
            }
    ...
    public function password_save(){
        ...
        if(IS_POST){
            ...
            $sqlstr = "update sys_admin set password='$password',loginpass='$loginpass' where id=".UID;
